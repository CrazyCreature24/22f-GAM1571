...
void ImGuiManager::StartFrame(float deltaTime)
{
    ...
    io.DisplaySize.x = (float)m_Framework->GetWindowWidth();
    io.DisplaySize.y = (float)m_Framework->GetWindowHeight();
    ...
    int mx, my;
    m_Framework->GetMouseCoordinates( &mx, &my );
    io.MousePos.x = (float)mx;
    io.MousePos.y = (float)my;
    io.MouseDown[0] = m_Framework->IsMouseButtonDown( 0 );
}
