...
bool FWCore::CreateGLWindow(char* title, int width, int height, unsigned char colorBits, unsigned char alphaBits, unsigned char zBits, unsigned char stencilBits, unsigned char multisampleSize, bool fullscreenflag)
{
    ...
    m_MyGLContext = new MyGLContext();
    if( m_MyGLContext->Create( m_hInstance, m_hDeviceContext, 4, 5, true, colorBits, alphaBits, zBits, stencilBits, multisampleSize ) == false )
    ...
    case WM_KEYDOWN:
        {
            ...
            if( keyWasPressedLastTimeMessageArrived == false )
            {
                ...
                Event* pEvent = new InputEvent( DeviceType::Keyboard, InputState::Pressed, (int)wParam );
    ...
    case WM_KEYUP:
        {
            ...
            Event* pEvent = new InputEvent( DeviceType::Keyboard, InputState::Released, (int)wParam );
            pFWCore->m_Game->GetEventManager()->AddEvent( pEvent );
        }
        return 0;
